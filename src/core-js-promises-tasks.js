...
function getPromise(number) {
  return new Promise((resolve, reject) => {
    if (number >= 0) {
      ...
      reject(new Error('The number must be non-negative.'));
    }
  });
}
...
function queuePromises(promises) {
  ...
  const finalPromise = promises.reduce(
    async (sequencePromise, currentPromise) => {
      const concatenatedResultSoFar = await sequencePromise;
      const currentResolvedValue = await currentPromise;
      return concatenatedResultSoFar + currentResolvedValue;
    },
    initialPromise
  );
